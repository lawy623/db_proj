...
def add_record():
    ...
    for i in range(5):
        ...
        if player != '':
            sql = """INSERT INTO score VALUES ({0},{1},{2},{3},{4},{5},{6})""".format(mid, "'"+nation+"'", level, "'"+home_cname+"'", player, number_score, own_goal)
            g.conn.execute(sql)
    ...
    for i in range(5):
        ...
        if player != '':
            sql = """INSERT INTO score VALUES ({0},{1},{2},{3},{4},{5},{6})""".format(mid, "'"+nation+"'", level, "'"+guest_cname+"'", player, number_score, own_goal)
            g.conn.execute(sql)
